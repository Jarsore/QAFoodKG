...
class QuestionPaser:
    ...
    def parser_main(self, res_classify):
        ...
        for question_type in question_types:
            ...
            if question_type == 'dish_restaurant':
                sql = self.sql_transfer(question_type, entity_dict.get('dish')) # sql_transfer是下面定义的分开处理问题子函数
        ...
        print(sqls)  # 有的明明在问句分类中已经分好类了，但是无法形成sql语句
    ...
    def sql_transfer(self, question_type, entities):
        ...
        #     # sql = ["MATCH (m:Restaurant)-[r:now_sell]->(n:Dish) where m.name = '{0}' return m.name, r.name, n.name".format(i) for i in entities]
            ...
            sql = ["MATCH (m:Dish) where m.name = '{0}' return m.name, m.chef".format(i) for i in entities]#调用match语句
            ...
            sql = ["MATCH (m:Dish) where m.name = '{0}' return m.name, m.cuisine".format(i) for i in entities]#调用match语句
            ...
            sql = ["MATCH (m:Dish) where m.name = '{0}' return m.name, m.flavor".format(i) for i in entities]#调用match语句
            # sql = ["MATCH (m:Dish)-[r:taste_like]->(n:Flavor) where m.name = '{0}' return m.name, r.name, n.name".format(i) for i in entities]

        elif question_type == 'dish_desc':
            sql = ["MATCH (m:Dish) where m.name = '{0}' return m.name, m.dec".format(i) for i in entities]  # 调用match语句
